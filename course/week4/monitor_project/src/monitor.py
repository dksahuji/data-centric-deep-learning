...
def get_hist_score(tr_probs, te_probs, bins=10):
  ...
  te_heights, te_bins = np.histogram(te_probs.cpu().numpy(), bins=tr_bins, density = True)
...
class MonitoringSystem:
  ...
  def calibrate(self, tr_probs, tr_labels, te_probs):
    ...
    # ============================
    calibrate_model = IsotonicRegression(out_of_bounds='clip')
    tr_probs_cal = calibrate_model.fit_transform(tr_probs.cpu().numpy(), tr_labels)
    te_probs_cal = calibrate_model.transform(te_probs.cpu().numpy())
